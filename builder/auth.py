...
from utils.dy_util import generate_webid, generate_fake_webid
...
class DouyinAuth:
    ...
    def perepare_auth(self, cookieStr: str, web_protect_: str = "", keys_: str = ""):
        ...
        # 更新 cookie_str，以便后续请求使用
        self.cookie_str = "; ".join([f"{k}={v}" for k, v in self.cookie.items()])

        # 如果缺少 s_v_web_id（verifyFp），尝试自动生成：
        # 优先通过网络从抖音页面获取真实 webid，失败时退回到伪造的数字 id
        if "s_v_web_id" not in self.cookie or not self.cookie.get("s_v_web_id"):
            try:
                webid = generate_webid(self)
                if webid:
                    self.cookie['s_v_web_id'] = webid
                else:
                    raise Exception("empty webid")
            except Exception:
                self.cookie['s_v_web_id'] = generate_fake_webid()

        # 重新构建 cookie_str，包含可能新加入的 s_v_web_id
        self.cookie_str = "; ".join([f"{k}={v}" for k, v in self.cookie.items()])
        if web_protect_ != "":
